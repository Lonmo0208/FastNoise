...
public final class FastChunkSection implements PaletteResizeListener<BlockState> {
  ...
  private int maxIdx = Integer.MIN_VALUE;
  ...
  public int onResize(int newBits, BlockState object) {
    ...
    if (canUseFastImport) fastImport(oldData, newData, newBits, this.maxIdx);
  ...
  private static <T> void fastImport(Data<T> oldData, Data<T> newData, int newBits, int upto) {
    ...
    else fastImport(oldStorage, newStorage, upto);
  }

  private static <T> void fastImport(
      PaletteStorage oldStorage, PaletteStorage newStorage, int upto) {
    var size = Math.min(upto, oldStorage.getSize());
    for (int i = 0; i < size; i++) newStorage.zenxarch$unsafeSet(i, oldStorage.get(i));
  }
